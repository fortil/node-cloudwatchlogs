...
var pathData = resolve(__dirname, "data.json");
...
var Logger = /** @class */ (function (_super) {
    ...
    Logger.config = function (loggerName, stream, config) {
        ...
        Loging.on('stackMessages', function stackMessages(type, first) {
            ...
            var LogStream = db.get("LOGGERS." + logName + "." + streamName).value(); // get(LOGGERS, `${logName}.${streamName}`)
            var LogStreamEvents = db.get("LOGGERS." + logName + "." + streamName + ".logEvents").value(); // get(LOGGERS, `${logName}.${streamName}.logEvents`)
        ...
        Loging.on('sendMessage', function sendMessage(element) {
            ...
            var sequenceToken = db.get("LOGGERS." + logName + "." + streamName + ".sequenceToken").value(); // get(LOGGERS, `${logName}.${streamName}.sequenceToken`)
        ...
        Loging.on('finishSendMessage', function finishSendMessage() {
            ...
            if (messages.length) {
                var element_2 = messages[0];
                console.log('====>finishSendMessage', element_2.name, ' | count', element_2.messages.length);
                // const totalMessages = parseInt((db.get('totalMessages') as any).getAllMessages().value());
                // set(LOGGERS, `totalMessages`, totalMessages - element.messages.length)
                db.set("LOGGERS." + element_2.name + "." + element_2.stream + ".logEvents", []).write();
                db.update('totalMessages', function (n) { return n - element_2.length; }).write();
                // set(LOGGERS, `${element.name}.${element.stream}.logEvents`, [])
                this.emit('sendMessage', element_2);
            }
            else {
                this._working = false;
